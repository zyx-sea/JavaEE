...
public interface TrxMapper {
    ...
    //根据id加入购物车
    @Insert("insert into trx(contentId,personId,price,time)values (#{content.id},#{user.id},#{content.price},#{time});")
    public void buy(@Param("content")Content content, @Param("time")String time, @Param("user")User user);
